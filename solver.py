...
import models
from utils import Bar, AverageMeter, mkdir_p
...
class Solver(object):
    def __init__(self, args):
        ...
        if self.args.resume:
            ...
            print("=> loaded checkpoint '{}'".format(self.args.resume))
    ...
    def set_optimizer(self):
        self.opt_g = torch.optim.SGD(self.g.parameters(),
                                     self.args.lr,
                                     momentum=self.args.momentum,
                                     weight_decay=self.args.weight_decay)

        self.opt_c1 = torch.optim.SGD(self.c1.parameters(),
                                      self.args.lr,
                                      momentum=self.args.momentum,
                                      weight_decay=self.args.weight_decay)

        self.opt_c2 = torch.optim.SGD(self.c2.parameters(),
                                      self.args.lr,
                                      momentum=self.args.momentum,
                                      weight_decay=self.args.weight_decay)
    ...
    def val(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(self.val_loader):
                ...
                # plot progress
                bar.suffix = '({batch}/{size}) Data: {data:.3f}s | Batch: \
                    {bt:.3f}s | Total: {total:} | ETA: {eta:}'.format(
                    batch=batch_idx + 1,
    ...
    def test(self, epoch):
        ...
        pickle.dump(
            dic, open(os.path.join(self.args.checkpoint, "results.p"), "wb"))
